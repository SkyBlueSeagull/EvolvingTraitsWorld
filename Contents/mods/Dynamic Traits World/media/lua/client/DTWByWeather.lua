...
local function outdoorsman() -- confirmed working
	...
	if player:isOutside() and player:getVehicle() == nil then
		...
		--print("DTW Logger: Outdoorsman totalGain="..totalGain..". OutdoorsmanCounter=" .. modData.OutdoorsmanCounter);
...
local function rainTraits() -- confirmed working
	...
	if rainIntensity > 0 and player:isOutside() and player:getVehicle() == nil then
		...
		local modData = player:getModData().DynamicTraitsWorld;
		--print("DTW Logger: rainTraits rainGain="..rainGain..". RainCounter=" .. modData.RainCounter);
